...
export default function LibraryPage() {
    ...
    useEffect(() => {
        ...
        // Save progress (Intro: 2 + S1-6: 56 = 58 pages total before Library)
        if (authed) {
            authSystem.updateReadingProgress(58 + pageNum)
        }
    }, [pageNum])
    ...
    const copyToClipboard = (sections: { label: string; text: string }[]) => {
        const promptSection = sections.find(s => s.label.includes("البرومبت"))
        if (promptSection) {
            navigator.clipboard.writeText(promptSection.text).then(() => {
                setShowToast(true)
