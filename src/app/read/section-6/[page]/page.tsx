...
import { section6, PageContent } from '@/data/bookData'
...
export default function Section6Page() {
    ...
    const [currentPage, setCurrentPage] = useState<PageContent | undefined>(section6[pageNum - 1])

    useEffect(() => {
        const page = section6[pageNum - 1]
        if (page) {
            setCurrentPage(page)
        } else {
            // If the page doesn't exist, go back to TOC
            if (params.page) {
                router.push('/toc')
            }
        }
  ...
  "ai_mode": "Builder"
                            ...
                            <div style={{
                                ...
                                <h4 style={{ color: '#FFB800', marginBottom: '12px', display: 'flex', alignItems: 'center', gap: '10px' }}>
                                    <span>🛠️</span> نصيحة القسم
                                </h4>
                                <p style={{ color: '#b0b0b0', fontSize: '0.95rem', lineHeight: '1.6', margin: 0 }}>
                                    الأداة هي وسيلة لتحقيق الهدف. اختر الأداة التي تناسب احتياجك التقني الآن، ولا تقع في فخ "تعلم الأداة للأبد". ابنِ، جرب، وانشر.
                                </p>
                            </div>
                        </div>
                    </div>
